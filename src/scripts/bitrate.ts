export async function run() {
  ...
  const bitrateSelect = document.querySelector<HTMLSelectElement>('.bitrate');
  bitrateSelect.addEventListener('change', (e) => {
    console.log('bitrateSelect change', e);
    const bitrate = bitrateSelect.options[bitrateSelect.selectedIndex].value;
    console.log('bitrate', bitrate);
    if (pc1.iceConnectionState == 'connected') {
      const senders = pc1.getSenders();
      console.log('pc1 senders', senders);
      for (const sender of senders) {
        if (sender.track.kind === 'video') {
          const parameters = sender.getParameters();
          console.log('parameters', parameters);
          const bandwidth = +bitrate;
          parameters.encodings[0].maxBitrate = bandwidth * 1000;
          sender.setParameters(parameters)
            .then(() => {
              console.log('Bitrate set');
            })
